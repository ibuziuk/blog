...
* Example: `{prod-url}#https://github.com/example/foo?che-editor=eclipse/che-theia/next` to use the next version of Theia (build after each commit) 
...
* Example: `{prod-url}#https://github.com/example/foo?df=mydevfile.yaml`


= URL syntax for workspace names

With DevWorkspace Operator enabled:
...
Each time you enter a URL such as `{prod-url}#http://github.com/foo/bar` again, {prod} reopens the previously created workspace `bar`. (If you deleted the previously created `bar` workspace, entering `{prod-url}#http://github.com/foo/bar` results in {prod} creating another workspace named `bar`.)
